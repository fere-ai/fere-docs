...
```bash
curl -X 'PUT' \
  'https://api.fereai.xyz/agent/3fa85f64-5717-4562-b3fc-2c963f66afa6/active/toggle' \
  -H 'accept: application/json'
```
</TabItem>
<TabItem value="python" label="Python">
...
def toggle_agent_status(api_key, agent_id):
    headers = {"Authorization": f"Bearer {api_key}"}
    response = requests.put(
        f"https://api.fereai.xyz/agent/{agent_id}/active/toggle",
        headers=headers
    )
    return response.json()
```
...
```bash
curl -X 'PUT' \
  'https://api.fereai.xyz/agent/edb296ad-3a7e-40e9-9e88-274e9c229010/dry-run/toggle' \
  -H 'accept: application/json'
...
def toggle_dry_run(api_key, agent_id):
    headers = {"Authorization": f"Bearer {api_key}"}
    response = requests.put(
        f"https://api.fereai.xyz/agent/{agent_id}/dry-run/toggle",
        headers=headers
    )
    if response.status_code == 200:
        return response.json()
...
```bash
curl -X 'GET' \
  'https://api.fereai.xyz/agent/3fa85f64-5717-4562-b3fc-2c963f66afa6/trades/?page=1' \
  -H 'accept: application/json'
```
</TabItem>
<TabItem value="python" label="Python" default>
...
def get_agent_trades(api_key, agent_id, page=1):
    headers = {"Authorization": f"Bearer {api_key}"}
    response = requests.get(
        f"https://api.fereai.xyz/agent/{agent_id}/trades/",
        params={"page": page},
        headers=headers
    )
    if response.status_code == 200:
...
async function getAgentTrades(apiKey, agentId, page = 1) {
    const url = `https://api.fereai.xyz/agent/${agentId}/trades/?page=${page}`;
    const headers = {
        'accept': 'application/json',
        'Authorization': `Bearer ${apiKey}`  // Include this if authentication is needed
    };
...
```bash
curl -X 'GET' \
  'https://api.fereai.xyz/agent/3fa85f64-5717-4562-b3fc-2c963f66afa6/holdings/' \
  -H 'accept: application/json'
```
</TabItem>
<TabItem value="python" label="Python" default>
...
def get_agent_holdings(api_key, agent_id):
    headers = {"Authorization": f"Bearer {api_key}"}
    response = requests.get(
        f"https://api.fereai.xyz/agent/{agent_id}/holdings/",
        headers=headers
    )
    if response.status_code == 200:
        return response.json()
...
```bash
curl -X 'GET' \
  'https://api.fereai.xyz/agent/3fa85f64-5717-4562-b3fc-2c963f66afa6/buy/' \
  -H 'accept: application/json'
```
</TabItem>
<TabItem value="python" label="Python" default>
...
def get_agent_buy_recommendations(api_key, agent_id):
    headers = {"Authorization": f"Bearer {api_key}"}
    response = requests.get(
        f"https://api.fereai.xyz/agent/{agent_id}/buy/",
        headers=headers
    )
    if response.status_code == 200:
        return response.json()
...
async function getBuySignals(apiKey, agent_id) {
    ...
    try {
        const response = await axios.get(`https://api.fereai.xyz/agents/{agent_id}buy/`, { headers });
        return response.data;
    } catch (error) {
