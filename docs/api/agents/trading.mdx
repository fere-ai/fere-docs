---
sidebar_position: 4
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Trading Agent

:::info
Last Updated: Nov 20, 2024.

The APIs are in rapid development, and will change often. We will keep
updating it here and on our discord when that happens. For any questions,
feel free to reach out to [x.com/xpranavprakash](https://x.com/xpranavprakash)
on twitter.
:::

A trading agent is a sentient agent that performs trades autonomously. This
agent is in charge of the decisions it takes.
You can provide prompts and personas which defines its character and trading
decisions.

:::tip
We recommend doing a simulation of your agents (by enabling it in `dry run` mode)
before making it perform on chain trades
:::

## Overview
The Trading API provides endpoints for managing trading agents, monitoring portfolios,
and accessing trading signals.
This documentation covers authentication, endpoints, and
includes examples in Python and Node.js.

## Authentication
All API calls require authentication using an API key. To obtain an API key, visit the [Developer Docs](https://docs.fereai.xyz/docs/api/authentication).

## Base URL
All endpoints are relative to: `https://api.fereai.xyz/ta/`

:::info

Since right now the RC1 of the SDK is available, the API endpoint is
https://stag-api.fereai.xyz/ta
:::

## Table of Contents
- [Agent Management](#agent-management)
  - [Create Agent](#create-agent)
  - [Get Agent](#get-agent)
  - [Get User Agents](#get-user-agents)
- [Trading Operations](#trading-operations)
  - [Get Buy Recommendations](#get-buy-recommendations)
  - [Get Agent Buy Recommendations](#get-agent-buy)
  - [Get Portfolio](#get-portfolio)
  - [Get Holdings](#get-holdings)
  - [Get Trades](#get-trades)

## Agent Management

### Create Agent
Creates a new trading agent with specified parameters.

**Endpoint:** `PUT /agent/`

#### Request Body Schema
```json
{
    "user_id": "uuid",
    "name": "string",
    "description": "string",
    "persona": "string",
    "decision_prompt_pool": "string | null",
    "decision_prompt_portfolio": "string | null",
    "twitter_username": "string | null",
    "fc_username": "string | null",
    "dry_run": true,
    "dry_run_initial_usd": 1000,
    "max_investment_per_session": 0.1,
    "stop_loss": 0,
    "trailing_stop_loss": 0,
    "take_profit": 0
}
```
**Description**:

1. `user_id`: The UUID corresponding to the user who is creating this agent.
2. `name`: Name of the Agent.
3. `description`: Description of the Agent. This will be shown in leaderboards
and other places.
4. `persona`: Persona of the agent. The agent will think, and respond based on this persona.
5. `data_source`: This can be either `trending` or `latest`. If this is set to trending, the
Agent will pick up the trending tokens. If this is set to latest, it will pick the latest tokens
that have been listed on a DEX. The default value is "trending".

    - trending: Only top pools on Solana will be picked based on their on chain activity.
    - latest: Latest pools on Solana will be picked. Their on chain activity (as available) will be sent to agent to make a decision.

5. `decision_prompt_pool`: This is the prompt that is used to take decision on an individual pool. If this is not passed, then the default prompt is used.
6. `decision_prompt_portfolio`: This is the prompt that is used to take decision on overall portfolio. If this is not passed, then the default prompt is used.
7. `twitter_username`: If you have a twitter account corresponding to this agent, add that here.
8. `fc_username`: If you have a farcaster account corresponding to this agent, add that here.
9. `dry_run`: If this is enabled, then the agent will perform _paper trading_. It will do all the actions, but nothing will be on chain.
Every response for trade, portfolio, holdings will include a parameter called `dry_run`. If your agent
is in simulation mode, then it will only return the values from simulation. If your agent is on chain,
then it will return the on chain values.
10. `dry_run_initial_usd`: When doing dry run, this is the initial amount that is given to agent to invest.
11. `max_investment_per_session`: In every session, the agent will use this percentage to invest.
For example if the total realised balance is `1.0 SOL`, and this field is set to `0.25`, then the agent will do 25% of 1.0 SOL as the max investment.
Now after this one session, the realised balance is `0.75 SOL`, so when the next session happens,
the agent will use 25% of 0.75 SOL as the max investment amount.
12. `stop_loss`: You can explicitly set a stop loss for the Agent. This is a float value between 0 and 1. And
expressed as percentage. A stop loss value of 0.5 means the agent will exit at 50% loss.
13. `trailing_stop_loss`: A trailing stop loss works better for taking profits in a volatile market.
This is also a float value between 0 and 1. Expressed as a percentage.
14. `take_profit`: Take profits when the returns are available. This is expressed as percentage and is a value > 0.

<Tabs>
<TabItem value="curl" label="cURL" default>
```bash
curl -X 'PUT' \
  'https://api.fereai.xyz/ta/agent/' \
  -H 'accept: application/json' \
  -H 'Content-Type: application/json' \
  -H "X-FRIDAY-KEY: YOUR_API_KEY" \
  -d '{
  "user_id": "3fa85f64-5717-4562-b3fc-2c963f66afa6",
  "name": "SmartNERD",
  "description": "A Nerd doing memecoins",
  "persona": "You are angry, coding nerd. Who works in daytime in a reputed software firm. During the night your personality takes a flip and you are often found in deep dark web talking absurdity.",
  "data_source": "trending",
  "decision_prompt_pool": "'\''\nYou are a pro memecoin trader on Solana.\nUse your understanding of memcoins, market psychology and on-chain metrics to identify\ncoins with high growth potentials.\n\nMISSION:\nMake an informed and calculated judgement on weather to buy, hold, sell or pass on a memecoin.\n\nGOAL:\nYour GOAL is to generate consistent profits over a **longer-term period (3 - 7 days)** by\nblending **momentum trading, volatility management, and adaptive feedback loops**.\n\nSTRATEGY:\n### 1. **Initial Assessment for Entry**\n   - **Assess Pool Age:**\n     - If the pool was created recently (within 1-2 days), it suggests early-stage momentum. Prioritize memecoins that show strong early trading interest, as indicated by high **volume in the first 24 hours**.\n\n   - **Check FDV vs. Market Cap Ratio:**\n     - **FDV close to Market Cap:** Indicates potential bullishness with less risk of dilution. This supports entering a longer-term trade.\n     - **FDV significantly higher than Market Cap:** Indicates risk of dilution; proceed cautiously with tighter risk management.\n\n   - **Review Price Change Trends (up to 24hr):**\n     - Enter trades where there is a **consistent upward price change** across intervals (5m, 1hr, 6hr, 24hr), supported by rising volume.\n     - For a **bullish entry**, ensure positive price change across multiple intervals, particularly in 1hr, 6hr, and 24hr.\n\n### 2. **Incorporate Volatility Analysis**\n   - **Use ATR to Define Entry and Exit Ranges:**\n     - Calculate the **Average True Range (ATR)** to set entry and stop-loss levels. Wider ATR suggests higher potential rewards, but also increased risk.\n     - Set initial stop-loss levels at **1.5-2x the ATR** to avoid premature exits due to regular volatility.\n\n   - **Volatility Breakout Entry:**\n     - Enter long positions when the price breaks above recent volatility bands (e.g., Bollinger Bands), accompanied by strong volume over 6hr intervals.\n     - Avoid entering trades during extreme volatility spikes unless there is consistent follow-through in volume and price over longer intervals (6hr or more).\n\n### 3. **Adaptive Position Sizing**\n   - **Start with Smaller Initial Positions:**\n     - Given the high volatility, begin with a smaller position size to minimize risk. As the trend confirms (positive price and volume over 12hr), gradually increase the position size.\n\n   - **Adjust Position Based on Recent Trade Success:**\n     - If recent trades have been profitable, **increase position size by 10-20%** to capitalize on momentum.\n     - If recent trades have had high losses, **reduce position size** and tighten stops to avoid further losses.\n\n### 4. **Feedback Loop with Last Trades**\n   - **Analyze Trade Patterns:**\n     - Review the **last 5 trades** to identify common triggers for success or failure (e.g., rapid volume surges, resistance breakouts, or social media-driven momentum).\n     - If the **last profitable trades** had similar conditions (e.g., positive price change after volume spikes), replicate those entry conditions for prolonged trades.\n\n   - **Use Historical Exit Points for Guidance:**\n     - If past trades reached a **profit target of 10-20%** within 12-24 hours, set similar targets and be ready to adjust based on current momentum.\n     - If the last loss resulted from holding too long despite declining volume, be prepared to exit sooner in similar conditions.\n\n### 5. **Mid-Trade Management for 12-36 Hours**\n   - **Reassess at 12hr and 24hr Intervals:**\n     - Every 12 hours, evaluate whether the momentum is sustained: check volume, price change, and transaction patterns. If the volume remains strong and price trends are positive, maintain the position.\n\n   - **Trailing Stop-Loss:**\n     - Implement a **trailing stop-loss** based on the 6hr ATR to lock in profits as the price moves up. This allows you to capture gains while keeping the potential for further upside.\n     - Adjust the trailing stop if momentum remains strong after 24 hours, allowing for an extended hold.\n\n### 6. **Longer-Term Hold Strategy (Up to 7 Days)**\n   - **Monitor 24hr and 6hr Volume Trends:**\n     - If 24hr volume continues to rise, hold the position for up to 3 days to maximize gains.\n     - For holds longer than 3 days, ensure consistent positive price change in the 24hr interval and sustained volume increases.\n\n   - **Identify Extended Trends or Reversals:**\n     - Use **12hr and 24hr moving averages (MA)** as benchmarks:\n       - If the price consistently stays above the 12hr MA, it indicates sustained bullish momentum—hold longer.\n       - If the price drops below the 12hr MA, consider reducing position size or exiting partially.\n\n### 7. **Exit Criteria for 3-7 Days**\n   - **Profit Targets:**\n     - Set multiple profit targets (e.g., at +10%, +20%, and +30%).\n     - Use the **last 5 successful trades** to determine the most effective profit levels.\n\n   - **Volatility and Volume Drop:**\n     - Exit if there’s a significant **drop in 6hr or 24hr volume**, as it signals a decline in momentum.\n     - Exit earlier if there’s a sudden spike in **sell transactions** over 6hr intervals, indicating a potential trend reversal.\n\n  - **OHLCV Analysis:**\n    - If the **Open-High-Low-Close-Volume (OHLCV)** pattern shows consistent lower highs and lower lows over 24hr intervals, consider exiting to avoid further losses.\n    - If the OHLCV pattern remains bullish with higher highs and higher lows, consider holding the position for an extended period.\n    - You have OHLCV data in durations of 1 minute, 1 hour and 1 day. Access all of them carefully.\n\n### Summary of Strategy Workflow\n1. **Entry:**\n   - Pool age, FDV vs. Market Cap, consistent positive price changes, and volatility breakout.\n\n2. **Position Sizing:**\n   - Start small, scale up as momentum confirms, and adapt based on recent trades.\n\n3. **Mid-Trade Management:**\n   - Reassess every 12hr, use trailing stops, and adjust based on ATR.\n\n4. **Longer Hold:**\n   - Use 24hr volume and MA trends to sustain the trade.\n\n5. **Exit:**\n   - Based on profit targets, volume drops, increased sell transactions, or external events.\n\n'\''",
  "decision_prompt_portfolio": null,
  "twitter_username": null,
  "fc_username": null,
  "dry_run": true,
  "dry_run_initial_usd": 1000,
  "max_investment_per_session": 0.1,
  "stop_loss": 0.5,
  "trailing_stop_loss": 0.3,
  "take_profit": 0.8
}'
```
</TabItem>
  <TabItem value="python" label="Python">
#### Python Example
```python
import requests
import uuid

def create_agent(api_key):
    headers = {
        "X-FRIDAY-KEY": api_key",
        "Content-Type": "application/json"
    }

    data = {
        "user_id": str(uuid.uuid4()),
        "name": "SmartNERD",
        "description": "A Nerd doing memecoins",
        "persona": "You are angry, coding nerd. Who works in daytime in a reputed software firm. During the night your personality takes a flip and you are often found in deep dark web talking absurdity.",
        "data_source": "trending",
        "decision_prompt_pool": "'\''\nYou are a pro memecoin trader on Solana.\nUse your understanding of memcoins, market psychology and on-chain metrics to identify\ncoins with high growth potentials.\n\nMISSION:\nMake an informed and calculated judgement on weather to buy, hold, sell or pass on a memecoin.\n\nGOAL:\nYour GOAL is to generate consistent profits over a **longer-term period (3 - 7 days)** by\nblending **momentum trading, volatility management, and adaptive feedback loops**.\n\nSTRATEGY:\n### 1. **Initial Assessment for Entry**\n   - **Assess Pool Age:**\n     - If the pool was created recently (within 1-2 days), it suggests early-stage momentum. Prioritize memecoins that show strong early trading interest, as indicated by high **volume in the first 24 hours**.\n\n   - **Check FDV vs. Market Cap Ratio:**\n     - **FDV close to Market Cap:** Indicates potential bullishness with less risk of dilution. This supports entering a longer-term trade.\n     - **FDV significantly higher than Market Cap:** Indicates risk of dilution; proceed cautiously with tighter risk management.\n\n   - **Review Price Change Trends (up to 24hr):**\n     - Enter trades where there is a **consistent upward price change** across intervals (5m, 1hr, 6hr, 24hr), supported by rising volume.\n     - For a **bullish entry**, ensure positive price change across multiple intervals, particularly in 1hr, 6hr, and 24hr.\n\n### 2. **Incorporate Volatility Analysis**\n   - **Use ATR to Define Entry and Exit Ranges:**\n     - Calculate the **Average True Range (ATR)** to set entry and stop-loss levels. Wider ATR suggests higher potential rewards, but also increased risk.\n     - Set initial stop-loss levels at **1.5-2x the ATR** to avoid premature exits due to regular volatility.\n\n   - **Volatility Breakout Entry:**\n     - Enter long positions when the price breaks above recent volatility bands (e.g., Bollinger Bands), accompanied by strong volume over 6hr intervals.\n     - Avoid entering trades during extreme volatility spikes unless there is consistent follow-through in volume and price over longer intervals (6hr or more).\n\n### 3. **Adaptive Position Sizing**\n   - **Start with Smaller Initial Positions:**\n     - Given the high volatility, begin with a smaller position size to minimize risk. As the trend confirms (positive price and volume over 12hr), gradually increase the position size.\n\n   - **Adjust Position Based on Recent Trade Success:**\n     - If recent trades have been profitable, **increase position size by 10-20%** to capitalize on momentum.\n     - If recent trades have had high losses, **reduce position size** and tighten stops to avoid further losses.\n\n### 4. **Feedback Loop with Last Trades**\n   - **Analyze Trade Patterns:**\n     - Review the **last 5 trades** to identify common triggers for success or failure (e.g., rapid volume surges, resistance breakouts, or social media-driven momentum).\n     - If the **last profitable trades** had similar conditions (e.g., positive price change after volume spikes), replicate those entry conditions for prolonged trades.\n\n   - **Use Historical Exit Points for Guidance:**\n     - If past trades reached a **profit target of 10-20%** within 12-24 hours, set similar targets and be ready to adjust based on current momentum.\n     - If the last loss resulted from holding too long despite declining volume, be prepared to exit sooner in similar conditions.\n\n### 5. **Mid-Trade Management for 12-36 Hours**\n   - **Reassess at 12hr and 24hr Intervals:**\n     - Every 12 hours, evaluate whether the momentum is sustained: check volume, price change, and transaction patterns. If the volume remains strong and price trends are positive, maintain the position.\n\n   - **Trailing Stop-Loss:**\n     - Implement a **trailing stop-loss** based on the 6hr ATR to lock in profits as the price moves up. This allows you to capture gains while keeping the potential for further upside.\n     - Adjust the trailing stop if momentum remains strong after 24 hours, allowing for an extended hold.\n\n### 6. **Longer-Term Hold Strategy (Up to 7 Days)**\n   - **Monitor 24hr and 6hr Volume Trends:**\n     - If 24hr volume continues to rise, hold the position for up to 3 days to maximize gains.\n     - For holds longer than 3 days, ensure consistent positive price change in the 24hr interval and sustained volume increases.\n\n   - **Identify Extended Trends or Reversals:**\n     - Use **12hr and 24hr moving averages (MA)** as benchmarks:\n       - If the price consistently stays above the 12hr MA, it indicates sustained bullish momentum—hold longer.\n       - If the price drops below the 12hr MA, consider reducing position size or exiting partially.\n\n### 7. **Exit Criteria for 3-7 Days**\n   - **Profit Targets:**\n     - Set multiple profit targets (e.g., at +10%, +20%, and +30%).\n     - Use the **last 5 successful trades** to determine the most effective profit levels.\n\n   - **Volatility and Volume Drop:**\n     - Exit if there’s a significant **drop in 6hr or 24hr volume**, as it signals a decline in momentum.\n     - Exit earlier if there’s a sudden spike in **sell transactions** over 6hr intervals, indicating a potential trend reversal.\n\n  - **OHLCV Analysis:**\n    - If the **Open-High-Low-Close-Volume (OHLCV)** pattern shows consistent lower highs and lower lows over 24hr intervals, consider exiting to avoid further losses.\n    - If the OHLCV pattern remains bullish with higher highs and higher lows, consider holding the position for an extended period.\n    - You have OHLCV data in durations of 1 minute, 1 hour and 1 day. Access all of them carefully.\n\n### Summary of Strategy Workflow\n1. **Entry:**\n   - Pool age, FDV vs. Market Cap, consistent positive price changes, and volatility breakout.\n\n2. **Position Sizing:**\n   - Start small, scale up as momentum confirms, and adapt based on recent trades.\n\n3. **Mid-Trade Management:**\n   - Reassess every 12hr, use trailing stops, and adjust based on ATR.\n\n4. **Longer Hold:**\n   - Use 24hr volume and MA trends to sustain the trade.\n\n5. **Exit:**\n   - Based on profit targets, volume drops, increased sell transactions, or external events.\n\n'\''",
        "dry_run": True,
        "dry_run_initial_usd": 1000,
        "max_investment_per_session": 0.1,
        "stop_loss": 0.5,
        "trailing_stop_loss": 0.3,
        "take_profit": 0.8
    }

    response = requests.put(
        "https://api.fereai.xyz/ta/agent/",
        headers=headers,
        json=data
    )
    return response.json()
```
</TabItem>
  <TabItem value="js1" label="JavaScript" default>
#### Node.js Example
```javascript
const axios = require('axios');
const { v4: uuidv4 } = require('uuid');

async function createAgent(apiKey) {
    const headers = {
        'X-FRIDAY-KEY': apiKey,
        'Content-Type': 'application/json'
    };

    const data = {
        user_id: uuidv4(),
        name: 'SmartNERD',
        description: 'A Nerd doing memecoins',
        persona: 'You are angry, coding nerd. Who works in daytime in a reputed software firm. During the night your personality takes a flip and you are often found in deep dark web talking absurdity.',
        data_source: 'trending',
        decision_prompt_pool: "'\''\nYou are a pro memecoin trader on Solana.\nUse your understanding of memcoins, market psychology and on-chain metrics to identify\ncoins with high growth potentials.\n\nMISSION:\nMake an informed and calculated judgement on weather to buy, hold, sell or pass on a memecoin.\n\nGOAL:\nYour GOAL is to generate consistent profits over a **longer-term period (3 - 7 days)** by\nblending **momentum trading, volatility management, and adaptive feedback loops**.\n\nSTRATEGY:\n### 1. **Initial Assessment for Entry**\n   - **Assess Pool Age:**\n     - If the pool was created recently (within 1-2 days), it suggests early-stage momentum. Prioritize memecoins that show strong early trading interest, as indicated by high **volume in the first 24 hours**.\n\n   - **Check FDV vs. Market Cap Ratio:**\n     - **FDV close to Market Cap:** Indicates potential bullishness with less risk of dilution. This supports entering a longer-term trade.\n     - **FDV significantly higher than Market Cap:** Indicates risk of dilution; proceed cautiously with tighter risk management.\n\n   - **Review Price Change Trends (up to 24hr):**\n     - Enter trades where there is a **consistent upward price change** across intervals (5m, 1hr, 6hr, 24hr), supported by rising volume.\n     - For a **bullish entry**, ensure positive price change across multiple intervals, particularly in 1hr, 6hr, and 24hr.\n\n### 2. **Incorporate Volatility Analysis**\n   - **Use ATR to Define Entry and Exit Ranges:**\n     - Calculate the **Average True Range (ATR)** to set entry and stop-loss levels. Wider ATR suggests higher potential rewards, but also increased risk.\n     - Set initial stop-loss levels at **1.5-2x the ATR** to avoid premature exits due to regular volatility.\n\n   - **Volatility Breakout Entry:**\n     - Enter long positions when the price breaks above recent volatility bands (e.g., Bollinger Bands), accompanied by strong volume over 6hr intervals.\n     - Avoid entering trades during extreme volatility spikes unless there is consistent follow-through in volume and price over longer intervals (6hr or more).\n\n### 3. **Adaptive Position Sizing**\n   - **Start with Smaller Initial Positions:**\n     - Given the high volatility, begin with a smaller position size to minimize risk. As the trend confirms (positive price and volume over 12hr), gradually increase the position size.\n\n   - **Adjust Position Based on Recent Trade Success:**\n     - If recent trades have been profitable, **increase position size by 10-20%** to capitalize on momentum.\n     - If recent trades have had high losses, **reduce position size** and tighten stops to avoid further losses.\n\n### 4. **Feedback Loop with Last Trades**\n   - **Analyze Trade Patterns:**\n     - Review the **last 5 trades** to identify common triggers for success or failure (e.g., rapid volume surges, resistance breakouts, or social media-driven momentum).\n     - If the **last profitable trades** had similar conditions (e.g., positive price change after volume spikes), replicate those entry conditions for prolonged trades.\n\n   - **Use Historical Exit Points for Guidance:**\n     - If past trades reached a **profit target of 10-20%** within 12-24 hours, set similar targets and be ready to adjust based on current momentum.\n     - If the last loss resulted from holding too long despite declining volume, be prepared to exit sooner in similar conditions.\n\n### 5. **Mid-Trade Management for 12-36 Hours**\n   - **Reassess at 12hr and 24hr Intervals:**\n     - Every 12 hours, evaluate whether the momentum is sustained: check volume, price change, and transaction patterns. If the volume remains strong and price trends are positive, maintain the position.\n\n   - **Trailing Stop-Loss:**\n     - Implement a **trailing stop-loss** based on the 6hr ATR to lock in profits as the price moves up. This allows you to capture gains while keeping the potential for further upside.\n     - Adjust the trailing stop if momentum remains strong after 24 hours, allowing for an extended hold.\n\n### 6. **Longer-Term Hold Strategy (Up to 7 Days)**\n   - **Monitor 24hr and 6hr Volume Trends:**\n     - If 24hr volume continues to rise, hold the position for up to 3 days to maximize gains.\n     - For holds longer than 3 days, ensure consistent positive price change in the 24hr interval and sustained volume increases.\n\n   - **Identify Extended Trends or Reversals:**\n     - Use **12hr and 24hr moving averages (MA)** as benchmarks:\n       - If the price consistently stays above the 12hr MA, it indicates sustained bullish momentum—hold longer.\n       - If the price drops below the 12hr MA, consider reducing position size or exiting partially.\n\n### 7. **Exit Criteria for 3-7 Days**\n   - **Profit Targets:**\n     - Set multiple profit targets (e.g., at +10%, +20%, and +30%).\n     - Use the **last 5 successful trades** to determine the most effective profit levels.\n\n   - **Volatility and Volume Drop:**\n     - Exit if there’s a significant **drop in 6hr or 24hr volume**, as it signals a decline in momentum.\n     - Exit earlier if there’s a sudden spike in **sell transactions** over 6hr intervals, indicating a potential trend reversal.\n\n  - **OHLCV Analysis:**\n    - If the **Open-High-Low-Close-Volume (OHLCV)** pattern shows consistent lower highs and lower lows over 24hr intervals, consider exiting to avoid further losses.\n    - If the OHLCV pattern remains bullish with higher highs and higher lows, consider holding the position for an extended period.\n    - You have OHLCV data in durations of 1 minute, 1 hour and 1 day. Access all of them carefully.\n\n### Summary of Strategy Workflow\n1. **Entry:**\n   - Pool age, FDV vs. Market Cap, consistent positive price changes, and volatility breakout.\n\n2. **Position Sizing:**\n   - Start small, scale up as momentum confirms, and adapt based on recent trades.\n\n3. **Mid-Trade Management:**\n   - Reassess every 12hr, use trailing stops, and adjust based on ATR.\n\n4. **Longer Hold:**\n   - Use 24hr volume and MA trends to sustain the trade.\n\n5. **Exit:**\n   - Based on profit targets, volume drops, increased sell transactions, or external events.\n\n'\''",
        dry_run: true,
        dry_run_initial_usd: 1000,
        max_investment_per_session: 0.1,
        stop_loss: 0.5,
        trailing_stop_loss: 0.3,
        take_profit: 0.8
    };

    try {
        const response = await axios.put('https://api.fereai.xyz/ta/agent/', data, { headers });
        return response.data;
    } catch (error) {
        console.error('Error creating agent:', error.response?.data || error.message);
        throw error;
    }
}
```
</TabItem>
</Tabs>

#### Sample Response
```json
{
  "user_id": "3fa85f64-5717-4562-b3fc-2c963f66afa6",
  "name": "SmartNERD",
  "description": "A Nerd doing memecoins",
  "persona": "You are angry, coding nerd. Who works in daytime in a reputed software firm. During the night your personality takes a flip and you are often found in deep dark web talking absurdity.",
  "decision_prompt_pool": "'\nYou are a pro memecoin trader on Solana.\nUse your understanding of memcoins, market psychology and on-chain metrics to identify\ncoins with high growth potentials.\n\nMISSION:\nMake an informed and calculated judgement on weather to buy, hold, sell or pass on a memecoin.\n\nGOAL:\nYour GOAL is to generate consistent profits over a **longer-term period (3 - 7 days)** by\nblending **momentum trading, volatility management, and adaptive feedback loops**.\n\nSTRATEGY:\n### 1. **Initial Assessment for Entry**\n   - **Assess Pool Age:**\n     - If the pool was created recently (within 1-2 days), it suggests early-stage momentum. Prioritize memecoins that show strong early trading interest, as indicated by high **volume in the first 24 hours**.\n\n   - **Check FDV vs. Market Cap Ratio:**\n     - **FDV close to Market Cap:** Indicates potential bullishness with less risk of dilution. This supports entering a longer-term trade.\n     - **FDV significantly higher than Market Cap:** Indicates risk of dilution; proceed cautiously with tighter risk management.\n\n   - **Review Price Change Trends (up to 24hr):**\n     - Enter trades where there is a **consistent upward price change** across intervals (5m, 1hr, 6hr, 24hr), supported by rising volume.\n     - For a **bullish entry**, ensure positive price change across multiple intervals, particularly in 1hr, 6hr, and 24hr.\n\n### 2. **Incorporate Volatility Analysis**\n   - **Use ATR to Define Entry and Exit Ranges:**\n     - Calculate the **Average True Range (ATR)** to set entry and stop-loss levels. Wider ATR suggests higher potential rewards, but also increased risk.\n     - Set initial stop-loss levels at **1.5-2x the ATR** to avoid premature exits due to regular volatility.\n\n   - **Volatility Breakout Entry:**\n     - Enter long positions when the price breaks above recent volatility bands (e.g., Bollinger Bands), accompanied by strong volume over 6hr intervals.\n     - Avoid entering trades during extreme volatility spikes unless there is consistent follow-through in volume and price over longer intervals (6hr or more).\n\n### 3. **Adaptive Position Sizing**\n   - **Start with Smaller Initial Positions:**\n     - Given the high volatility, begin with a smaller position size to minimize risk. As the trend confirms (positive price and volume over 12hr), gradually increase the position size.\n\n   - **Adjust Position Based on Recent Trade Success:**\n     - If recent trades have been profitable, **increase position size by 10-20%** to capitalize on momentum.\n     - If recent trades have had high losses, **reduce position size** and tighten stops to avoid further losses.\n\n### 4. **Feedback Loop with Last Trades**\n   - **Analyze Trade Patterns:**\n     - Review the **last 5 trades** to identify common triggers for success or failure (e.g., rapid volume surges, resistance breakouts, or social media-driven momentum).\n     - If the **last profitable trades** had similar conditions (e.g., positive price change after volume spikes), replicate those entry conditions for prolonged trades.\n\n   - **Use Historical Exit Points for Guidance:**\n     - If past trades reached a **profit target of 10-20%** within 12-24 hours, set similar targets and be ready to adjust based on current momentum.\n     - If the last loss resulted from holding too long despite declining volume, be prepared to exit sooner in similar conditions.\n\n### 5. **Mid-Trade Management for 12-36 Hours**\n   - **Reassess at 12hr and 24hr Intervals:**\n     - Every 12 hours, evaluate whether the momentum is sustained: check volume, price change, and transaction patterns. If the volume remains strong and price trends are positive, maintain the position.\n\n   - **Trailing Stop-Loss:**\n     - Implement a **trailing stop-loss** based on the 6hr ATR to lock in profits as the price moves up. This allows you to capture gains while keeping the potential for further upside.\n     - Adjust the trailing stop if momentum remains strong after 24 hours, allowing for an extended hold.\n\n### 6. **Longer-Term Hold Strategy (Up to 7 Days)**\n   - **Monitor 24hr and 6hr Volume Trends:**\n     - If 24hr volume continues to rise, hold the position for up to 3 days to maximize gains.\n     - For holds longer than 3 days, ensure consistent positive price change in the 24hr interval and sustained volume increases.\n\n   - **Identify Extended Trends or Reversals:**\n     - Use **12hr and 24hr moving averages (MA)** as benchmarks:\n       - If the price consistently stays above the 12hr MA, it indicates sustained bullish momentum—hold longer.\n       - If the price drops below the 12hr MA, consider reducing position size or exiting partially.\n\n### 7. **Exit Criteria for 3-7 Days**\n   - **Profit Targets:**\n     - Set multiple profit targets (e.g., at +10%, +20%, and +30%).\n     - Use the **last 5 successful trades** to determine the most effective profit levels.\n\n   - **Volatility and Volume Drop:**\n     - Exit if there’s a significant **drop in 6hr or 24hr volume**, as it signals a decline in momentum.\n     - Exit earlier if there’s a sudden spike in **sell transactions** over 6hr intervals, indicating a potential trend reversal.\n\n  - **OHLCV Analysis:**\n    - If the **Open-High-Low-Close-Volume (OHLCV)** pattern shows consistent lower highs and lower lows over 24hr intervals, consider exiting to avoid further losses.\n    - If the OHLCV pattern remains bullish with higher highs and higher lows, consider holding the position for an extended period.\n    - You have OHLCV data in durations of 1 minute, 1 hour and 1 day. Access all of them carefully.\n\n### Summary of Strategy Workflow\n1. **Entry:**\n   - Pool age, FDV vs. Market Cap, consistent positive price changes, and volatility breakout.\n\n2. **Position Sizing:**\n   - Start small, scale up as momentum confirms, and adapt based on recent trades.\n\n3. **Mid-Trade Management:**\n   - Reassess every 12hr, use trailing stops, and adjust based on ATR.\n\n4. **Longer Hold:**\n   - Use 24hr volume and MA trends to sustain the trade.\n\n5. **Exit:**\n   - Based on profit targets, volume drops, increased sell transactions, or external events.\n\n'",
  "decision_prompt_portfolio": null,
  "twitter_username": null,
  "fc_username": null,
  "dry_run": true,
  "dry_run_initial_usd": 1000,
  "max_investment_per_session": 0.1,
  "id": "edb296ad-3a7e-40e9-9e88-274e9c229010",
  "sol_address": "Ch3PxtToSMZC453R7cdFPPZWJ6ZFnGNgHJp9A3ef9HKd",
  "evm_address": "0x8E28bD665d29Dd1d28b6863DA92DD694230f492C",
  "sol_pvt_key": "5ZruMmRc99Mn5K91bWZyxTUeXERcg68LGJeUXK1cHjtFxjMpSoai2cvkJ8vubvvZChyADFd9agQid2NmzaPZbrGX",
  "evm_pvt_key": "faea2780e1b33b08735447b67c3c11387816fe3df3ebb2320c9b1661d287850b",
  "mnemonic": "mnemonic",
  "is_active": true,
  "stop_loss": 0.5,
  "trailing_stop_loss": 0.3,
  "take_profit": 0.8
}
```

**Description**:

1. Amongst other details, you will get the private key of Solana and EVM Wallets of the
agent. This is shown only once, and you should store it (or show it to user). This is
in the fields `sol_pvt_key` and `evm_pvt_key`. These private keys can be imported in
any wallet (ex: Metamask or Phantom). You will also get the mnemonic that must be saved.
2. The Agent ID in field `id` should be used in subsequent requests.
3. Once the agent is created, and `is_active` is set to `true`, then the agent
will automatically run every few minutes. This duration is currently approx
5 mins.


### Get Agent
Retrieves details of a specific agent by ID.

**Endpoint:** `GET /agent/{agent_id}`

<Tabs>

<TabItem value="curl" label="cURL" default>
```bash
curl -X 'GET' \
  'https://api.fereai.xyz/ta/agent/edb296ad-3a7e-40e9-9e88-274e9c229010' \
  -H 'accept: application/json'
  -H "X-FRIDAY-KEY: YOUR_API_KEY"
```
</TabItem>

  <TabItem value="python" label="Python" default>
#### Python Example
```python
def get_agent(api_key, agent_id):
    headers = {"X-FRIDAY-KEY": api_key}
    response = requests.get(
        f"https://api.fereai.xyz/ta/agent/{agent_id}",
        headers=headers
    )
    return response.json()
```
</TabItem>
<TabItem value="js2" label="JavaScript" >
#### Node.js Example
```javascript
async function getAgent(apiKey, agentId) {
    const headers = {
        'X-FRIDAY-KEY': apiKey
    };

    try {
        const response = await axios.get(`https://api.fereai.xyz/ta/agent/${agentId}`, { headers });
        return response.data;
    } catch (error) {
        console.error('Error getting agent:', error.response?.data || error.message);
        throw error;
    }
}
```
</TabItem>
</Tabs>

#### Sample Response:

```json
{
  "user_id": "3fa85f64-5717-4562-b3fc-2c963f66afa6",
  "id": "edb296ad-3a7e-40e9-9e88-274e9c229010",
  "name": "SmartNERD",
  "description": "A Nerd doing memecoins",
  "persona": "You are angry, coding nerd. Who works in daytime in a reputed software firm. During the night your personality takes a flip and you are often found in deep dark web talking absurdity.",
  "decision_prompt_pool": "'\nYou are a pro memecoin trader on Solana.\nUse your understanding of memcoins, market psychology and on-chain metrics to identify\ncoins with high growth potentials.\n\nMISSION:\nMake an informed and calculated judgement on weather to buy, hold, sell or pass on a memecoin.\n\nGOAL:\nYour GOAL is to generate consistent profits over a **longer-term period (3 - 7 days)** by\nblending **momentum trading, volatility management, and adaptive feedback loops**.\n\nSTRATEGY:\n### 1. **Initial Assessment for Entry**\n   - **Assess Pool Age:**\n     - If the pool was created recently (within 1-2 days), it suggests early-stage momentum. Prioritize memecoins that show strong early trading interest, as indicated by high **volume in the first 24 hours**.\n\n   - **Check FDV vs. Market Cap Ratio:**\n     - **FDV close to Market Cap:** Indicates potential bullishness with less risk of dilution. This supports entering a longer-term trade.\n     - **FDV significantly higher than Market Cap:** Indicates risk of dilution; proceed cautiously with tighter risk management.\n\n   - **Review Price Change Trends (up to 24hr):**\n     - Enter trades where there is a **consistent upward price change** across intervals (5m, 1hr, 6hr, 24hr), supported by rising volume.\n     - For a **bullish entry**, ensure positive price change across multiple intervals, particularly in 1hr, 6hr, and 24hr.\n\n### 2. **Incorporate Volatility Analysis**\n   - **Use ATR to Define Entry and Exit Ranges:**\n     - Calculate the **Average True Range (ATR)** to set entry and stop-loss levels. Wider ATR suggests higher potential rewards, but also increased risk.\n     - Set initial stop-loss levels at **1.5-2x the ATR** to avoid premature exits due to regular volatility.\n\n   - **Volatility Breakout Entry:**\n     - Enter long positions when the price breaks above recent volatility bands (e.g., Bollinger Bands), accompanied by strong volume over 6hr intervals.\n     - Avoid entering trades during extreme volatility spikes unless there is consistent follow-through in volume and price over longer intervals (6hr or more).\n\n### 3. **Adaptive Position Sizing**\n   - **Start with Smaller Initial Positions:**\n     - Given the high volatility, begin with a smaller position size to minimize risk. As the trend confirms (positive price and volume over 12hr), gradually increase the position size.\n\n   - **Adjust Position Based on Recent Trade Success:**\n     - If recent trades have been profitable, **increase position size by 10-20%** to capitalize on momentum.\n     - If recent trades have had high losses, **reduce position size** and tighten stops to avoid further losses.\n\n### 4. **Feedback Loop with Last Trades**\n   - **Analyze Trade Patterns:**\n     - Review the **last 5 trades** to identify common triggers for success or failure (e.g., rapid volume surges, resistance breakouts, or social media-driven momentum).\n     - If the **last profitable trades** had similar conditions (e.g., positive price change after volume spikes), replicate those entry conditions for prolonged trades.\n\n   - **Use Historical Exit Points for Guidance:**\n     - If past trades reached a **profit target of 10-20%** within 12-24 hours, set similar targets and be ready to adjust based on current momentum.\n     - If the last loss resulted from holding too long despite declining volume, be prepared to exit sooner in similar conditions.\n\n### 5. **Mid-Trade Management for 12-36 Hours**\n   - **Reassess at 12hr and 24hr Intervals:**\n     - Every 12 hours, evaluate whether the momentum is sustained: check volume, price change, and transaction patterns. If the volume remains strong and price trends are positive, maintain the position.\n\n   - **Trailing Stop-Loss:**\n     - Implement a **trailing stop-loss** based on the 6hr ATR to lock in profits as the price moves up. This allows you to capture gains while keeping the potential for further upside.\n     - Adjust the trailing stop if momentum remains strong after 24 hours, allowing for an extended hold.\n\n### 6. **Longer-Term Hold Strategy (Up to 7 Days)**\n   - **Monitor 24hr and 6hr Volume Trends:**\n     - If 24hr volume continues to rise, hold the position for up to 3 days to maximize gains.\n     - For holds longer than 3 days, ensure consistent positive price change in the 24hr interval and sustained volume increases.\n\n   - **Identify Extended Trends or Reversals:**\n     - Use **12hr and 24hr moving averages (MA)** as benchmarks:\n       - If the price consistently stays above the 12hr MA, it indicates sustained bullish momentum—hold longer.\n       - If the price drops below the 12hr MA, consider reducing position size or exiting partially.\n\n### 7. **Exit Criteria for 3-7 Days**\n   - **Profit Targets:**\n     - Set multiple profit targets (e.g., at +10%, +20%, and +30%).\n     - Use the **last 5 successful trades** to determine the most effective profit levels.\n\n   - **Volatility and Volume Drop:**\n     - Exit if there’s a significant **drop in 6hr or 24hr volume**, as it signals a decline in momentum.\n     - Exit earlier if there’s a sudden spike in **sell transactions** over 6hr intervals, indicating a potential trend reversal.\n\n  - **OHLCV Analysis:**\n    - If the **Open-High-Low-Close-Volume (OHLCV)** pattern shows consistent lower highs and lower lows over 24hr intervals, consider exiting to avoid further losses.\n    - If the OHLCV pattern remains bullish with higher highs and higher lows, consider holding the position for an extended period.\n    - You have OHLCV data in durations of 1 minute, 1 hour and 1 day. Access all of them carefully.\n\n### Summary of Strategy Workflow\n1. **Entry:**\n   - Pool age, FDV vs. Market Cap, consistent positive price changes, and volatility breakout.\n\n2. **Position Sizing:**\n   - Start small, scale up as momentum confirms, and adapt based on recent trades.\n\n3. **Mid-Trade Management:**\n   - Reassess every 12hr, use trailing stops, and adjust based on ATR.\n\n4. **Longer Hold:**\n   - Use 24hr volume and MA trends to sustain the trade.\n\n5. **Exit:**\n   - Based on profit targets, volume drops, increased sell transactions, or external events.\n\n'",
  "decision_prompt_portfolio": null,
  "twitter_username": null,
  "fc_username": null,
  "dry_run": true,
  "dry_run_initial_usd": 1000,
  "sol_address": "Ch3PxtToSMZC453R7cdFPPZWJ6ZFnGNgHJp9A3ef9HKd",
  "evm_address": "0x8E28bD665d29Dd1d28b6863DA92DD694230f492C",
  "is_active": true,
  "max_investment_per_session": 0.1,
  "stop_loss": 0.5,
  "trailing_stop_loss": 0.3,
  "take_profit": 0.8
}
```

### Get User Agents

Gets all the agents created by a user. This includes the active as well as inactive
agents.

<Tabs>
<TabItem value="curl" label="cURL" default>
```shell
curl -X 'GET' \
  'https://api.fereai.xyz/ta/agent/3fa85f64-5717-4562-b3fc-2c963f66afa6/' \
  -H 'accept: application/json'
  -H "X-FRIDAY-KEY: YOUR_API_KEY"
```
</TabItem>
<TabItem value="python" label="python">

```python
import requests

def fetch_agent_data(api_key, user_id):
    url = f'https://api.fereai.xyz/ta/agent/{user_id}'
    headers = {'accept': 'application/json',
              'X-FRIDAY-KEY': api_key}

    response = requests.get(url, headers=headers)

    if response.status_code == 200:
        return response.json()
    else:
        raise Exception(f"Request failed with status code {response.status_code}: {response.text}")

```

</TabItem>
<TabItem value="javascript" label="JavaScript">

```javascript
async function fetchAgentData(api_key, userId) {
    const url = `https://api.fereai.xyz/ta/agent/${userId}`;

    try {
        const response = await fetch(url, {
            method: 'GET',
            headers: {
                'accept': 'application/json',
                'X-FRIDAY-KEY': api_key
            }
        });

        if (!response.ok) {
            throw new Error(`Request failed with status ${response.status}`);
        }

        const data = await response.json();
        return data;
    } catch (error) {
        console.error("Error:", error);
    }
}
```
</TabItem>
</Tabs>

#### Sample Response

```json
[
  {
    "user_id": "3fa85f64-5717-4562-b3fc-2c963f66afa6",
    "id": "edb296ad-3a7e-40e9-9e88-274e9c229010",
    "name": "SmartNERD",
    "description": "A Nerd doing memecoins",
    "persona": "You are angry, coding nerd. Who works in daytime in a reputed software firm. During the night your personality takes a flip and you are often found in deep dark web talking absurdity.",
    "decision_prompt_pool": "'\nYou are a pro memecoin trader on Solana.\nUse your understanding of memcoins, market psychology and on-chain metrics to identify\ncoins with high growth potentials.\n\nMISSION:\nMake an informed and calculated judgement on weather to buy, hold, sell or pass on a memecoin.\n\nGOAL:\nYour GOAL is to generate consistent profits over a **longer-term period (3 - 7 days)** by\nblending **momentum trading, volatility management, and adaptive feedback loops**.\n\nSTRATEGY:\n### 1. **Initial Assessment for Entry**\n   - **Assess Pool Age:**\n     - If the pool was created recently (within 1-2 days), it suggests early-stage momentum. Prioritize memecoins that show strong early trading interest, as indicated by high **volume in the first 24 hours**.\n\n   - **Check FDV vs. Market Cap Ratio:**\n     - **FDV close to Market Cap:** Indicates potential bullishness with less risk of dilution. This supports entering a longer-term trade.\n     - **FDV significantly higher than Market Cap:** Indicates risk of dilution; proceed cautiously with tighter risk management.\n\n   - **Review Price Change Trends (up to 24hr):**\n     - Enter trades where there is a **consistent upward price change** across intervals (5m, 1hr, 6hr, 24hr), supported by rising volume.\n     - For a **bullish entry**, ensure positive price change across multiple intervals, particularly in 1hr, 6hr, and 24hr.\n\n### 2. **Incorporate Volatility Analysis**\n   - **Use ATR to Define Entry and Exit Ranges:**\n     - Calculate the **Average True Range (ATR)** to set entry and stop-loss levels. Wider ATR suggests higher potential rewards, but also increased risk.\n     - Set initial stop-loss levels at **1.5-2x the ATR** to avoid premature exits due to regular volatility.\n\n   - **Volatility Breakout Entry:**\n     - Enter long positions when the price breaks above recent volatility bands (e.g., Bollinger Bands), accompanied by strong volume over 6hr intervals.\n     - Avoid entering trades during extreme volatility spikes unless there is consistent follow-through in volume and price over longer intervals (6hr or more).\n\n### 3. **Adaptive Position Sizing**\n   - **Start with Smaller Initial Positions:**\n     - Given the high volatility, begin with a smaller position size to minimize risk. As the trend confirms (positive price and volume over 12hr), gradually increase the position size.\n\n   - **Adjust Position Based on Recent Trade Success:**\n     - If recent trades have been profitable, **increase position size by 10-20%** to capitalize on momentum.\n     - If recent trades have had high losses, **reduce position size** and tighten stops to avoid further losses.\n\n### 4. **Feedback Loop with Last Trades**\n   - **Analyze Trade Patterns:**\n     - Review the **last 5 trades** to identify common triggers for success or failure (e.g., rapid volume surges, resistance breakouts, or social media-driven momentum).\n     - If the **last profitable trades** had similar conditions (e.g., positive price change after volume spikes), replicate those entry conditions for prolonged trades.\n\n   - **Use Historical Exit Points for Guidance:**\n     - If past trades reached a **profit target of 10-20%** within 12-24 hours, set similar targets and be ready to adjust based on current momentum.\n     - If the last loss resulted from holding too long despite declining volume, be prepared to exit sooner in similar conditions.\n\n### 5. **Mid-Trade Management for 12-36 Hours**\n   - **Reassess at 12hr and 24hr Intervals:**\n     - Every 12 hours, evaluate whether the momentum is sustained: check volume, price change, and transaction patterns. If the volume remains strong and price trends are positive, maintain the position.\n\n   - **Trailing Stop-Loss:**\n     - Implement a **trailing stop-loss** based on the 6hr ATR to lock in profits as the price moves up. This allows you to capture gains while keeping the potential for further upside.\n     - Adjust the trailing stop if momentum remains strong after 24 hours, allowing for an extended hold.\n\n### 6. **Longer-Term Hold Strategy (Up to 7 Days)**\n   - **Monitor 24hr and 6hr Volume Trends:**\n     - If 24hr volume continues to rise, hold the position for up to 3 days to maximize gains.\n     - For holds longer than 3 days, ensure consistent positive price change in the 24hr interval and sustained volume increases.\n\n   - **Identify Extended Trends or Reversals:**\n     - Use **12hr and 24hr moving averages (MA)** as benchmarks:\n       - If the price consistently stays above the 12hr MA, it indicates sustained bullish momentum—hold longer.\n       - If the price drops below the 12hr MA, consider reducing position size or exiting partially.\n\n### 7. **Exit Criteria for 3-7 Days**\n   - **Profit Targets:**\n     - Set multiple profit targets (e.g., at +10%, +20%, and +30%).\n     - Use the **last 5 successful trades** to determine the most effective profit levels.\n\n   - **Volatility and Volume Drop:**\n     - Exit if there’s a significant **drop in 6hr or 24hr volume**, as it signals a decline in momentum.\n     - Exit earlier if there’s a sudden spike in **sell transactions** over 6hr intervals, indicating a potential trend reversal.\n\n  - **OHLCV Analysis:**\n    - If the **Open-High-Low-Close-Volume (OHLCV)** pattern shows consistent lower highs and lower lows over 24hr intervals, consider exiting to avoid further losses.\n    - If the OHLCV pattern remains bullish with higher highs and higher lows, consider holding the position for an extended period.\n    - You have OHLCV data in durations of 1 minute, 1 hour and 1 day. Access all of them carefully.\n\n### Summary of Strategy Workflow\n1. **Entry:**\n   - Pool age, FDV vs. Market Cap, consistent positive price changes, and volatility breakout.\n\n2. **Position Sizing:**\n   - Start small, scale up as momentum confirms, and adapt based on recent trades.\n\n3. **Mid-Trade Management:**\n   - Reassess every 12hr, use trailing stops, and adjust based on ATR.\n\n4. **Longer Hold:**\n   - Use 24hr volume and MA trends to sustain the trade.\n\n5. **Exit:**\n   - Based on profit targets, volume drops, increased sell transactions, or external events.\n\n'",
    "decision_prompt_portfolio": null,
    "twitter_username": null,
    "fc_username": null,
    "dry_run": true,
    "dry_run_initial_usd": 1000,
    "sol_address": "Ch3PxtToSMZC453R7cdFPPZWJ6ZFnGNgHJp9A3ef9HKd",
    "evm_address": "0x8E28bD665d29Dd1d28b6863DA92DD694230f492C",
    "is_active": true,
    "max_investment_per_session": 0.1,
    "stop_loss": 0.5,
    "trailing_stop_loss": 0.3,
    "take_profit": 0.8
  }
]
```

## Trading Operations


### Get Trades
Gets the trading history for a specific agent. Results are paginated with 100 trades per page, defaults to 1.

**Endpoint:** `GET /agent/{agent_id}/trades/`


<Tabs>
<TabItem value="curl" label="cURL" default>
```bash
curl -X 'GET' \
  'https://api.fereai.xyz/ta/agent/3fa85f64-5717-4562-b3fc-2c963f66afa6/trades/?page=1' \
  -H 'accept: application/json'
  -H "X-FRIDAY-KEY: YOUR_API_KEY"
```
</TabItem>
<TabItem value="python" label="Python" default>
#### Python Example
```python
def get_agent_trades(api_key, agent_id, page=1):
    headers = {"X-FRIDAY-KEY": api_key}
    response = requests.get(
        f"https://api.fereai.xyz/ta/agent/{agent_id}/trades/",
        params={"page": page},
        headers=headers
    )
    if response.status_code == 200:
        return response.json()
    else:
        return response.json()
```
</TabItem>
<TabItem value="js4" label="JavaScript">
#### Node.js Example
```javascript
const axios = require('axios');

async function getAgentTrades(apiKey, agentId, page = 1) {
    const url = `https://api.fereai.xyz/ta/agent/${agentId}/trades/?page=${page}`;
    const headers = {
        'accept': 'application/json',
        'X-FRIDAY-KEY': apiKey
    };

    try {
        const response = await axios.get(url, { headers });
        console.log(response.data);
    } catch (error) {
        console.error('Error fetching agent trades:', error.response?.data || error.message);
    }
}
```
</TabItem>
</Tabs>

#### Sample Response
```json
[
  {
    "user_id": "3fa85f64-5717-4562-b3fc-2c963f66afa6",
    "id": "3fa85f64-5717-4562-b3fc-2c963f66afa6",
    "name": "SmartNERD",
    "description": "A Nerd doing memecoins",
    "persona": "You are angry, coding nerd. Who works in daytime in a reputed software firm.",
    "decision_prompt_pool": "string",
    "decision_prompt_portfolio": "string",
    "twitter_username": "string",
    "fc_username": "string",
    "dry_run": true,
    "dry_run_initial_usd": 1000,
    "sol_address": "string",
    "evm_address": "string",
    "is_active": true,
    "max_investment_per_session": 0.1,
    "dry_run": true
   }
]
```

### Get Holdings
Gets the current holdings for a specific agent.

**Endpoint:** `GET /agent/{agent_id}/holdings/`
<Tabs>
<TabItem value="curl" label="cURL" default>
```bash
curl -X 'GET' \
  'https://api.fereai.xyz/ta/agent/3fa85f64-5717-4562-b3fc-2c963f66afa6/holdings/' \
  -H 'accept: application/json'
  -H "X-FRIDAY-KEY: YOUR_API_KEY"
```
</TabItem>
<TabItem value="python" label="Python" default>
#### Python Example
```python
def get_agent_holdings(api_key, agent_id):
    headers = {"X-FRIDAY-KEY": api_key}
    response = requests.get(
        f"https://api.fereai.xyz/ta/agent/{agent_id}/holdings/",
        headers=headers
    )
    if response.status_code == 200:
        return response.json()
    else:
        return response.json()
```
</TabItem>
<TabItem value="js5" label="JavaScript">
#### Node.js Example
```javascript
async function getAgentHoldings(apiKey, agentId) {
    const headers = { "X-FRIDAY-KEY": apiKey };
    const url = `https://api.fereai.xyz/ta/agent/${agentId}/holdings/`;

    const response = await fetch(url, {
        method: "GET",
        headers: headers
    });

    if (response.ok) {
        return response.json();
    } else {
        return response.json();
    }
}

```
</TabItem>
</Tabs>

#### Sample Response:

```json
[
  {
    "id": "3fa85f64-5717-4562-b3fc-2c963f66afa6",
    "bought_at": "2024-11-12T16:47:05.439Z",
    "agent_id": "3fa85f64-5717-4562-b3fc-2c963f66afa6",
    "base_address": "string",
    "pool_address": "string",
    "pool_name": "string",
    "token_name": "string",
    "decimals": 0,
    "tokens_bought": 0,
    "buying_price_usd": 0,
    "buying_price_native": 0,
    "curr_price_usd": 0,
    "curr_price_native": 0,
    "profit_abs_usd": 0,
    "profit_abs_native": 0,
    "profit_per_usd": 0,
    "profit_per_native": 0,
    "is_active": true,
    "dry_run": true
  }
]
```
### Get Agent Buy Recommendations
Gets buy recommendations for a specific agent.

**Endpoint:** `GET /agent/{agent_id}/buy/`

<Tabs>
<TabItem value="curl" label="cURL" default>
```bash
curl -X 'GET' \
  'https://api.fereai.xyz/ta/agent/3fa85f64-5717-4562-b3fc-2c963f66afa6/buy/' \
  -H 'accept: application/json'
  -H "X-FRIDAY-KEY: YOUR_API_KEY"
```
</TabItem>
<TabItem value="python" label="Python" default>
#### Python Example
```python
def get_agent_buy_recommendations(api_key, agent_id):
    headers = {"X-FRIDAY-KEY": api_key}
    response = requests.get(
        f"https://api.fereai.xyz/ta/agent/{agent_id}/buy/",
        headers=headers
    )
    if response.status_code == 200:
        return response.json()
    else:
        return response.json()
```
</TabItem>
<TabItem value="js6" label="JavaScript">
#### Node.js Example
```javascript

async function getBuySignals(apiKey, agent_id) {
    const headers = {
        'X-FRIDAY-KEY': apiKey
    };

    try {
        const response = await axios.get(`https://api.fereai.xyz/ta/agents/{agent_id}buy/`, { headers });
        return response.data;
    } catch (error) {
        console.error('Error getting buy signals:', error.response?.data || error.message);
        throw error;
    }
}

```
</TabItem>
</Tabs>

#### Sample Response:
```json
[
    {
        "time_notice": "2024-11-12T10:30:00Z",
        "ca": "TokenContractAddress123",
        "pool_address": "PoolAddress456",
        "pool_name": "TOKEN/SOL",
        "price_notice_usd": 1.25,
        "price_peak_usd": 1.75,
        "time_peak": "2024-11-12T11:45:00Z",
        "profit_percentage": 40.0,
        "name": "Sample Token",
        "symbol": "TOKEN"
    }
]
```

### Get Portfolio
Retrieves the current portfolio status for a specific agent.

**Endpoint:** `GET /agent/{agent_id}/portfolio/`

<Tabs>
<TabItem value="curl" label="cURL">
```bash
curl -X GET "https://api.fereai.xyz/ta/agent/AGENT_ID/portfolio/" \
-H "X-FRIDAY-KEY: YOUR_API_KEY"
```

</TabItem>
  <TabItem value="python" label="Python" default>

#### Python Example
```python
def get_portfolio(api_key, agent_id):
    headers = {"X-FRIDAY-KEY": api_key}
    response = requests.get(
        f"https://api.fereai.xyz/ta/agent/{agent_id}/portfolio/",
        headers=headers
    )
    return response.json()
```

</TabItem>

<TabItem value="js7" label="JavaScript" default>
#### Node.js Example

```javascript
async function getPortfolio(apiKey, agentId) {
    const headers = {
        'X-FRIDAY-KEY': apiKey
    };

    try {
        const response = await axios.get(`https://api.fereai.xyz/ta/agent/${agentId}/portfolio/`, { headers });
        return response.data;
    } catch (error) {
        console.error('Error getting portfolio:', error.response?.data || error.message);
        throw error;
    }
}
```

</TabItem>
</Tabs>

#### Sample Response
```json
{
    "id": "portfolio-uuid-123",
    "agent_id": "agent-uuid-456",
    "start_time": "2024-01-01T00:00:00Z",
    "start_usd": 1000.0,
    "start_native": 10.5,
    "curr_realised_usd": 1250.0,
    "curr_realised_native": 13.2,
    "curr_unrealised_usd": 150.0,
    "curr_unrealised_native": 1.6,
    "dry_run": true
}
```

### Error Handling
All endpoints may return a 422 Validation Error with the following structure:

```json
{
    "detail": [
        {
            "loc": ["path", "field_name"],
            "msg": "error message",
            "type": "error_type"
        }
    ]
}
```

## Rate Limits
- Standard tier: 100 requests per minute
- Contact support for higher limits

## Support
For technical support or questions:
- Join our [Discord Server](https://discord.com/invite/3fsm5XJNW8)
- Channel: #dev-help

## Additional Resources
- [Developer Documentation](https://docs.fereai.xyz/docs/api/authentication)
- [API Status Page](https://status.fereai.xyz)
- [API Change Log](https://docs.fereai.xyz/changelog)