...
Toggles the active status of a specific agent. When activated, the agent automatically runs every few minutes

**Endpoint:** `PUT /agent/{agent_id}/active/toggle`
...
```bash
curl -X 'PUT' \
  'https://api.fereai.xyz/ta/agent/3fa85f64-5717-4562-b3fc-2c963f66afa6/active/toggle' \
  -H 'accept: application/json'
  -H "X-FRIDAY-KEY: YOUR_API_KEY"
```
</TabItem>
<TabItem value="python" label="Python">
```python
def toggle_agent_status(api_key, agent_id):
    headers = {"X-FRIDAY-KEY": api_key}
    response = requests.put(
        f"https://api.fereai.xyz/ta/agent/{agent_id}/active/toggle",
        headers=headers
    )
    return response.json()
```
...
<TabItem value="javascript" label="JavaScript">
```javascript
async function toggleAgentStatus(apiKey, agentId) {
    const headers = { "X-FRIDAY-KEY": apiKey };
    const url = `https://api.fereai.xyz/ta/agent/${agentId}/active/toggle`;
...
Toggles the dry run status of a specific agent.

**Endpoint:** `PUT /agent/{agent_id}/dry-run/toggle`

<Tabs>
<TabItem value="curl" label="cURL" default>
```bash
curl -X 'PUT' \
  'https://api.fereai.xyz/ta/agent/edb296ad-3a7e-40e9-9e88-274e9c229010/dry-run/toggle' \
  -H 'accept: application/json'
  -H "X-FRIDAY-KEY: YOUR_API_KEY"
...
```python
def toggle_dry_run(api_key, agent_id):
    headers = {"X-FRIDAY-KEY": api_key}
    response = requests.put(
        f"https://api.fereai.xyz/ta/agent/{agent_id}/dry-run/toggle",
        headers=headers
    )
    if response.status_code == 200:
        return response.json()
...
```javascript
async function toggleDryRun(apiKey, agentId) {
    const headers = { "X-FRIDAY-KEY": apiKey };
    const url = `https://api.fereai.xyz/ta/agent/${agentId}/dry-run/toggle`;
